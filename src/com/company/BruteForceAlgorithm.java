...
public class BruteForceAlgorithm {
    ...
    public void count_ways(){
        ...
        initialization_brute(usable_values, size, sum);
        System.out.println("The algorithm find " + this.ways + " different ways.");
    }

    private void initialization_brute(List<Integer> arr, int size, int brute_size) {
        List<Integer> chosen = new ArrayList<>();
        recursive_brute(chosen, arr, 0, brute_size, 0, size - 1);
    }

    private void recursive_brute(List<Integer> chosen, List<Integer> arr, int index, int brute_size, int start, int end) {
        int sum_of_line = 0;
        if (index == brute_size) {
            for (int i = 0; i < brute_size; i++) {
                if (arr.get(chosen.get(i)) != 0)
                    System.out.printf("%d ", arr.get(chosen.get(i)));
                sum_of_line = sum_of_line + arr.get(chosen.get(i));
            }
            if (sum_of_line == sum){
        ...
        for (int i = start; i <= end; i++) {
            chosen.add(index,i);
            recursive_brute(chosen, arr, index + 1, brute_size, i, end);
        }
    }
}
